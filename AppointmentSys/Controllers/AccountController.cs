...
namespace AppointmentSys.Controllers
{
    ...
    { 
        ...
        {
            int rolesCount = 0;
            object cachedObjectData = ContextCache.Get("TotalRoles");
            if (cachedObjectData != null)
            {
                rolesCount = (int)cachedObjectData;

            }
            else
            {
                rolesCount = RoleManager.Roles.Count(); ;
                ContextCache.MaxOne("TotalRoles", rolesCount);
            }
            return rolesCount;
        }
        ...
        {
            int userCount = 0;
            object cachedObjectData = ContextCache.Get("TotalUserAdminAsync");
            if (cachedObjectData != null)
            {
                userCount = (int)cachedObjectData;

            }
            else
            {
                var userAdmin = await UserManager.Users.ToListAsync();
                userCount = userAdmin.Count();
                ContextCache.MaxOne("TotalUserAdminAsync", userCount);
            }
                ...
                if (result.Succeeded)
                {
                    ...
                    string body = "Dear " + model.Email + "," +
                       "\n\nWelcome to AppointmentSys!" +
                        "\n\nA request has been received to open your AppointmentSys account." +
                        "\n\nPlease confirm your account by clicking this link: <a href=\"" + callbackUrl + "\">Click here</a>." +

                        "\n\nIf you did not initiate this request, please contact us immediately at support@AppointmentSys.com." +
                        "\n\nThank you," +
                        "\nThe AppointmentSys Team.";
